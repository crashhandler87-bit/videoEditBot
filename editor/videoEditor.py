...
def edit(file, groupData, par, workingDir = "", resourceDir = "..", toVideo = False, toGif = False, disallowTimecodeBreak = False, HIDE_FFMPEG_OUT = True, HIDE_ALL_FFMPEG = True, SHOW_TIMER = False, fixPrint = fixPrint):
    videoFX = ['playreverse', 'hmirror', 'vmirror', 'lag', 'rlag', 'shake', 'fisheye', 'defisheye', 'zoom', 'bottomtext', 'toptext', 'normalcaption', 'topcap', 'bottomcap', 'topcaption', 'bottomcaption', 'hypercam', 'bandicam', 'kinemaster', 'deepfry', 'contrast', 'hue', 'hcycle', 'speed', 'vreverse', 'areverse', 'reverse', 'wscale', 'hscale', 'sharpen', 'watermark', 'framerate', 'invert', 'gm4', 'invertred', 'invertgreen', 'invertblue', 'wave', 'waveamount', 'wavestrength', 'acid', 'hcrop', 'vcrop', 'hflip', 'vflip']
    ...
    if any(orderedVideoFX):
        ...
        def defisheye():
            nonlocal video, audio
            d['defisheye'] = int(constrain(d['fisheye'], 1, 2))
    ...
    if any(orderedAudioFX):
        ...
        def pitch(SOXCMD, AUDPRE):
            d['pitch'] = 12 * int(constrain(d['pitch'], -12000, 12000))
...
def videoEdit(originalFile, args, workingDir = "./", resourceDir = path.dirname(__file__), disallowTimecodeBreak = False, keepExtraFiles = False, SHOW_TIMER = False, HIDE_FFMPEG_OUT = True, HIDE_ALL_FFMPEG = True, fixPrint = fixPrint, durationUnder = None, allowRandom = True, logErrors = False):
    ...
    par = {
        ...
        "invertgreen"        :[V, "invgreen" , 1 ],
        "invertblue"        :[V, "invblue" , 1 ],
        "gm4"        :[V, "g4" , 1 ],
